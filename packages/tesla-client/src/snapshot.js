...
export async function saveSnapshot(db, vehicleId, { state, ts, raw }) {
  ...
  await db.query(
    ...
    [
      ...
      toKm(chargeState.battery_range) ?? null,
      toKm(driveState.speed) ?? null,
      driveState.latitude ?? null,
      driveState.longitude ?? null,
      driveState.heading ?? null,
      toKm(vehicleState.odometer) ?? null,
      ...
      JSON.stringify({ df: vehicleState.df, dr: vehicleState.dr, pf: vehicleState.pf, pr: vehicleState.pr }),
      JSON.stringify({

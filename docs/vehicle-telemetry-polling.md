...
**Cost:** the fast-follow window isn't free — during those 10 minutes the worker makes ~6-7 due-checks instead of the ~0 the old cadence would have made in the same window, each costing 2 billed calls (lite + full poll, both gated through `checkAndConsumeBudget`), so roughly **+13 billed Fleet API calls per trip** on top of normal usage. `docs/setup-tesla-api.md` estimates baseline usage (pre-fast-follow) at ~4,050 requests/month, comfortably inside Tesla's $10/month free discount for typical single-vehicle use (see `docs/setup-tesla-api.md` and [Tesla's Billing and Limits docs](https://developer.tesla.com/docs/fleet-api/billing-and-limits)) — this adds noticeably to that (order of +30-40% for a couple of round trips a day). Two independent safety nets if that ever runs past the free discount: Tesla's own billing limit defaults to $0 and *cannot* charge you without a payment method explicitly added to the developer account (usage is suspended instead), and `TESLA_FAST_FOLLOW_ENABLED=false` disables this behavior entirely and falls back to the flat 15-minute `online` cadence (pre-fix behavior) — see Env vars below.
...
| `BATTERY_CAPACITY_KWH` | `worker/src/handlers/charging.js` | `56` | Flat battery capacity (kWh) used to estimate `energy_added_kwh` when a charging session closes: `((endLevel - startLevel) / 100) * BATTERY_CAPACITY_KWH`. One global value for the whole fleet — not per vehicle/trim, not read from Tesla's own charge telemetry. |
| `TESLA_FAST_FOLLOW_ENABLED` | `worker/src/stateMachine.js` | `true` | Set to `"false"` to disable the fast-follow interval above and go back to the flat 15-minute `online`/idle cadence. The fast-follow window costs roughly +13 billed calls per trip (see above) — this is the knob to turn it off if that's pushing you over your Fleet API free-tier quota. Read once at module import time, same caveat as the two above. |

All three are read once at module import time — an unset `TESLA_MAX_CALLS_PER_DAY`/`BATTERY_CAPACITY_KWH` falls back to its default via `Number(process.env.X || default)`, and an unparsable value becomes `NaN` with no validation; `TESLA_FAST_FOLLOW_ENABLED` is a string compared against `"false"`, so anything other than exactly `"false"` (including unset) keeps it enabled.

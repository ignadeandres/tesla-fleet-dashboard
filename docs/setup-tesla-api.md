...
- Free tier: $10/month discount per account (per Tesla's [Billing and Limits](https://developer.tesla.com/docs/fleet-api/billing-and-limits) docs — corrected from an earlier, wrong $14 figure). Estimated usage for this project's smart-polling design: ~4,050 Data requests/month before the fast-follow interval (`docs/vehicle-telemetry-polling.md`), more with it enabled — within the free discount for typical single-vehicle personal use, but not a guarantee; monitor actual usage on the Billing and Usage page.
- **Your account's billing limit defaults to $0 and stays there until you explicitly add a payment method.** Per Tesla's docs: "Applications that exceed their billing limit or do not have a configured payment method will be automatically disabled." With no payment method on file, exceeding the free discount suspends API access until the next billing cycle — it cannot silently charge you. If you do add a payment method, set an explicit billing limit on the developer dashboard as a hard cap.

...
**Feature:** User-facing charging session history table (data capture is documented separately at `docs/vehicle-telemetry-polling/`)
**Status:** Already implemented and shipped. Elaborates `docs/charging-sessions/business-requirements.md`.

## Functional Behavior

### Scope
Page: `/v/:vehicleId/charging` — read-only table of a vehicle's charging session history. All behavior below is derived from `ChargingPage.jsx`, the `VEHICLE_CHARGING_QUERY` GraphQL query, and the `chargingSessions` resolver chain (`requireOwnedVehicle` → `getChargingSessionsByVehicle`).
...
4. On success with ≥1 session, page renders a table with columns **Start**, **Duration**, **Battery**, **Energy added** — one row per session, in the order returned (most recent charge first).
...
- **Duration**:
  ...
  - If `endTime` is null (session still open): literal text `"in progress"`, not a computed value. This value is static — it does not update live while the page is open; refreshing the page is required to see an updated duration once the underlying data changes.
...
**Behavior:**
- No table, no headers, no empty-table shell is rendered.
- Single line of secondary/muted text: **"No charging sessions recorded yet."**
- This state is mutually exclusive with the loading state and the table state — it only renders once loading has finished and the resolved session list length is exactly 0.
...
1. **Cold load (no cached data yet):**
   ...
   - Behavior: render a spinner (CircularProgress) in place of any table or empty-state text. No column headers, no partial table.
...
2. **Background refetch with existing cache (e.g., Apollo cache-and-network re-fetch, tab refocus, etc.):**
   ...
   - Behavior: continue showing the previously loaded table (or the previously resolved empty state) as-is. No spinner is shown, and no loading affordance overlays the stale table. The user has no visual indication a refetch is in progress; the table simply updates in place once the refetch resolves (or stays stale silently if it fails — see error handling below, which is not specifically re-triggered by a background refetch failure in this component).

**Transition rule:** As soon as `loading` becomes `false` and `data` is populated, the page re-evaluates to either the empty state (Story 2) or the table (Story 1) based on session count — never both, never neither.
...
- No in-page navigation exists — there is no row click, drill-down, or modal, so the table itself is a dead end (unlike Trips, which exits to a per-trip map view).

### 1. First load, sessions present
1. User lands on `/v/:vehicleId/charging` (via tab click or direct link).
2. `VEHICLE_CHARGING_QUERY` fires with `{ id: vehicleId, limit: 50 }`; no cached data yet → `loading && !data` is true → a `CircularProgress` spinner is shown in place of the table.
3. Query resolves → spinner is replaced by the table, sessions rendered most-recent-first, one row per session, no pagination/filter/sort controls.
4. Screen is now static; user reads the table or navigates away via a tab.

### 2. First load, zero sessions
1. Same as above through query resolution.
2. `sessions.length === 0` → table is never rendered; instead a single line of secondary-colored text is shown: "No charging sessions recorded yet."
...
- Apollo Client's default `cache-first` policy (no explicit `fetchPolicy` on this query, `frontend/src/graphql/queries/charging.js` + `frontend/src/pages/ChargingPage.jsx`) means: if the exact `{id, limit}` variables were already cached from the prior visit, the table renders immediately from cache with no spinner and no network round-trip — it does not re-fetch fresh data on tab return within the same session.
- Switching vehicles produces new query variables, so it's treated as a fresh load (spinner or empty state as in flows 1/2) rather than a cache hit.
...
- No separate screen/state — it's a row-level rendering rule within the same table, not a navigation state:

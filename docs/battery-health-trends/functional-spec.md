...
5. Chart renders as an `AreaChart` (redesigned from a bare `Line` — same one series, same interpolation, now with a gradient fill under the line so the chart reads as "level filled up to here" rather than an abstract line): y-axis domain fixed `[0, 100]` with `%` unit (not derived from data min/max), x-axis uses `tsLabel`, one `Area` series (`batteryLevel`) stroked in the app's charge-accent color (`theme.tokens.charge` — cobalt blue, mode-dependent: brighter in dark mode, deeper in light mode) with a fading fill beneath it, no dot markers, monotone interpolation between points. Axis ticks, gridlines, and the tooltip card use the app's hairline/mono-text tokens instead of Recharts defaults, also mode-dependent.
